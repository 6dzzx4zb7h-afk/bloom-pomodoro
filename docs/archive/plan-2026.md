# Bloom — Evidence-Informed Iterative Build Plan (ARCHIVED)

> **This document is archived history, retired August 3, 2026. It is not the current plan.**
>
> Active work lives in [`ROADMAP.md`](../../ROADMAP.md). The binding constraints that outlived this
> plan live in [`CLAUDE.md`](../../CLAUDE.md) and the `docs/` authorities it cites.
>
> **Why it was retired:** it did its job. 73 of 93 numbered steps shipped, including every feature
> phase (0–6), the July 2026 remediation pass, data stewardship, day plans, and the native iOS
> wrapper. What remained was either finished-but-unticked, blocked on unavailable tooling, or scoped
> for a cross-device sync feature that has since been dropped in favor of staying local-only.
>
> **One correction was applied on archiving.** Steps 9.3, 9.5, 9.6, 10.4 and 10.8 were still
> unchecked here but had in fact shipped in commit `2025284` ("Ship Phase 10 planner, foundations,
> and history") — with implementations, tests, and schema migrations v28→v30. Their boxes are now
> ticked so this record is accurate. Nothing else was rewritten; the 20 boxes still unchecked below
> were genuinely never completed, and `ROADMAP.md` records which of them were kept, dropped, or
> deliberately deferred.
>
> Read this file when you want to know *why* something in Bloom is the way it is. The per-step
> `Science:`, `Quality:` and `Closed (…)` notes are the reasoning and the verification evidence
> behind decisions that are now just code.
...
### - [x] 10.4 Day plan UI — two taps from suggestion to plan
...
### - [x] 10.8 Foundations card — one tap, where idle moments already live
